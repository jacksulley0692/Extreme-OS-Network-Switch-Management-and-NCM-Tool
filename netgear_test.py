        ...
        print("[*] Data collected. Disconnected from switch. Analyzing raw text...\n")

        # 4. Parse Forwarding Database (FDB)
        # Matches MAC layouts and grabs the trailing port string (e.g., 00:14:6c:aa:bb:cc ... 12)
        port_mac_map = {}
        fdb_pattern = r'([0-9a-fA-F:]{17})\s+\S+\s+(?:\[\w+\])?\s+(\d+)'
        fdb_matches = re.findall(fdb_pattern, fdb_output)
        
        for mac, port in fdb_matches:
            mac = mac.lower()
            if port not in port_mac_map:
                port_mac_map[port] = []
            if mac not in port_mac_map[port]:
                port_mac_map[port].append(mac)

        # 5. Parse LLDP Context
        # Splits the output down into per-port blocks to grab specific system text strings
        lldp_data = {}
        ...
        for section in port_sections[1:]:
            ...
            port = lines[0].strip().split()[0]  # Extracts the port number string safely
            sys_name = ""
            sys_desc = ""
        ...
        for port, macs in port_mac_map.items():
            # Filter out active trunk/LAG groups to eliminate false positives
            ...
            # Check for Netgear MAC address signatures (Common Netgear OUIs)
            netgear_ouis = ['00:14:6c', '00:26:f2', '84:1b:5e', '00:18:4d', '00:2b:21']
            for mac in macs:
                if any(mac.startswith(oui) for oui in netgear_ouis):
                    is_netgear_mac = True
                    reasons.append(f"MAC OUI Signature matches Netgear hardware profiles.")
                    break

            # Check LLDP fields for the string "netgear"
            if port in lldp_data:
                lldp_info = lldp_data[port]
                if "netgear" in lldp_info['name'] or "netgear" in lldp_info['desc']:
                    is_netgear_lldp = True
                    reasons.append(f"LLDP details explicitly announce vendor name 'NETGEAR'.")
