...
def run_test():
    print("=== Extreme-OS Netgear Discovery Test Script (EDP Guardrail v6) ===")
    ...
    try:
        ...
        for line in edp_output.splitlines():
            # Strips line and checks if it starts with an active integer port assignment column layout
            line_clean = line.strip()
            port_match = re.match(r'^(\d+(?::\d+)?)', line_clean)
            if port_match:
                raw_port = port_match.group(1)
                clean_port = raw_port.split(':')[-1]
                edp_ports.add(clean_port)
        
        print(f"[*] Diagnostic Context: Active EDP Core Uplink Ports isolated -> {list(edp_ports)}")
        ...
        # 3. PARSE LLDP (Fixed list string context parsing)
        lldp_data = {}
        ...
        if "LLDP Port " in lldp_normalized:
            ...
            for section in port_sections[1:]:
                lines_in_section = section.splitlines()
                if not lines_in_section: 
                    continue
                
                # FIX: Read the first item of the string array natively to find the port number header integer
                first_line = lines_in_section[0].strip()
        ...
        for port in all_active_ports:
            # Drop known link aggregated LAG trunks instantly
            if port in sharing_output:
                continue

            # THE EDP GUARDRAIL
            # If the port is found in your show edp neighbor table index, it's an official Extreme node trunk!
