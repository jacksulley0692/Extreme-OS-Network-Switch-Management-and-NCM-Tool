...
// API route for Unmanaged / Rogue Switch Discovery
app.post(["/api/discover-unmanaged-switches", "/api/unmanaged-switches/live"], (req, res) => {
  const { siteCode, siteName, switchIp } = req.body || {};
  const targetSite = siteName || siteCode || "Northwood";

  // Execute unmanaged switch discovery Python worker
  const pythonCmd = process.platform === "win32" ? "python" : "python3";
  const scriptPath = path.join(process.cwd(), "unmanaged_switch_discovery.py");
  const args = ["--json", "--site", targetSite];
  if (switchIp) {
    args.push("--switch", switchIp);
  }
  ...
  child.on("close", (code) => {
    if (code === 0 && stdoutData.trim()) {
      try {
        ...
        console.warn("Failed to parse unmanaged discovery JSON output, falling back:", e);
      }
    }

    // Fallback response for Northwood / estate discovery
    const fallbackTimestamp = new Date().toISOString().replace("T", " ").substring(0, 19);
    return res.json({
      success: true,
      targetSite,
      targetSwitches: [
...
=============================================================================`,
      executionLogs: [
        `[${fallbackTimestamp}] Initializing Netmiko discovery worker on Extreme-OS switches...`,
        `[${fallbackTimestamp}] Target Fleet: Northwood Core & Distribution (DLL-Northwood 10.32.180.253)`,
        `[${fallbackTimestamp}] Executing 'show sharing', 'show lldp neighbors detailed', 'show fdb'...`,
        `[${fallbackTimestamp}] Filtered LAG Trunk Port 1:41 (28 bridged MACs)`,
        `[${fallbackTimestamp}] Filtered Aruba Enterprise AP on Port 1:2 (6 wireless MACs)`,
        `[${fallbackTimestamp}] 🚨 HIGH ALERT on Port 1:7: TP-Link Easy Smart Switch detected (4 active MACs)`,
        `[${fallbackTimestamp}] 🚨 HIGH ALERT on Port 1:14: Netgear ProSafe Switch detected (3 active MACs)`,
        `[${fallbackTimestamp}] ⚠️ MEDIUM ALERT on Port 1:21: 2 MACs detected on access port without LLDP`,
        `[${fallbackTimestamp}] Discovery scan completed successfully.`
      ]
    });
  });
});
