...
"""
===============================================================================
BackupSave.py - Automated Extreme Networks Switch Configuration Backup Engine
===============================================================================

Supported Switch OS Families:
  - ExtremeXOS (EXOS): Summit X440, X450, X460, X670, 5000 Series
  - VOSS (VSP): Virtual Operating System (VSP 4000, 7000, 8000 Series Fabric Engines)

Core Workflow:
  1. Reads target switch inventory from Switches.txt / switches.txt
  2. Resolves TFTP and SSH/Telnet credentials from config.ini / conf.ini
  3. Pre-checks reachability via ICMP / TCP port probes
  4. Connects via Netmiko / Telnet / SSH
  5. Executes pre-backup configuration save:
       - EXOS: 'save configuration' -> confirms with 'y'
       - VOSS: 'save config' or 'copy config nvram'
  6. Exports active configuration to TFTP server:
       - EXOS: 'tftp put <TFTP_IP> vr "VR-Default" <FILENAME.xsf>'
       - VOSS: 'copy config tftp address <TFTP_IP> filename <FILENAME.cfg>'
  7. Updates real-time telemetry into status.json and status.txt for the web portal
  8. Generates structured audit reports in reports/ and logs/

CLI Usage:
  python BackupSave.py                    # Back up all switches in Switches.txt
  python BackupSave.py --switch 10.32.81.250 # Back up a single switch
  python BackupSave.py --all              # Explicit full estate backup
===============================================================================
"""
import sys
import argparse
...
# Flexible discovery for Switches.txt and config/conf.ini
SWITCH_CANDIDATES = [
...
def get_cfg(section, key, fallback=None):
    # Try specified section
    if config.has_section(section) and config.has_option(section, key):
        return config.get(section, key)
    # Try lowercase/uppercase section variants
...
try:
    hop_val = get_cfg("hop", "enabled", fallback="true")
    HOP_ENABLED = str(hop_val).lower() in ["true", "1", "yes", "on"]
except Exception:
    HOP_ENABLED = True

try:
    HOP_SUBNET_PREFIX = int(get_cfg("hop", "subnet_prefix", fallback="24") or "24")
except Exception:
    HOP_SUBNET_PREFIX = 24

HOP_MGMT_VR = get_cfg("hop", "mgmt_vr", fallback=TFTP_VR)

try:
    HOP_TIMEOUT = int(get_cfg("hop", "timeout", fallback="25") or "25")
except Exception:
    HOP_TIMEOUT = 25

try:
    RETRY_DELAY = int(get_cfg("backup", "retry_delay", fallback="5") or "5")
except Exception:
    RETRY_DELAY = 5

try:
    VERIFY_WAIT = int(get_cfg("backup", "verify_wait", fallback="30") or "30")
except Exception:
    VERIFY_WAIT = 30
...
log_file = LOG_FOLDER / f"switch_backup_save_{today}.log"
csv_report = REPORT_FOLDER / f"switch_backup_save_report_{run_timestamp}.csv"
...
def save_switch_configuration(connection, ip):
    """Executes 'save configuration primary' on ExtremeXOS and responds to confirmation prompts."""
    logging.info(f"[{ip}] Executing 'save configuration primary' before backup...")
    try:
        output = connection.send_command_timing("save configuration primary", delay_factor=2)
        if "y/N" in output or "yes/no" in output.lower() or "?" in output:
            output += connection.send_command_timing("y", delay_factor=2)
        logging.info(f"[{ip}] Save output: {output.strip()}")
        return True, output.strip()
    except Exception as e:
        logging.error(f"[{ip}] Failed to save configuration: {e}")
        return False, str(e)
...
def base_result(ip, **overrides):
    result = {
        ...
        "backup_file": "",
        "file_verified": "No",
        "unsaved_changes": "No (Saved before backup)",
        "message": ""
    }
...
def backup_switch(ip):
    ...
    try:
        ...
        had_unsaved = prompt_shows_unsaved(current_prompt)
        if had_unsaved:
            logging.info(f"[{ip}] Switch currently has unsaved changes (* prompt detected).")

        # MANDATORY SAVE STEP
        save_ok, save_msg = save_switch_configuration(connection, ip)
        ...
        if has_bad_words(output):
            return base_result(
                ...
                ping="Yes",
                port_open="Yes",
                config_saved="Yes" if save_ok else "Failed",
                backup_status="Failed",
                backup_file=backup_filename,
                file_verified=backup_file_exists(backup_filename),
                unsaved_changes="Saved",
        ...
        if verified == "No":
            status = "Warning"
            message = "Configuration saved and upload triggered, but file not found in configured TFTP root"
        else:
            status = "Success"
            message = "Configuration saved & backup completed" + (" (succeeded on retry)" if retried else "")
...
def backup_switch_via_hop(target_ip, intermediary_ip):
    ...
    try:
        ...
        # MANDATORY SAVE STEP VIA HOP
        logging.info(f"[{target_ip}] (via {intermediary_ip}) Executing 'save configuration primary'...")
        connection.write_channel("save configuration primary\n")
        time.sleep(2)
        save_out = ""
        try:
            save_out = connection.read_channel()
        except Exception:
            pass
        if "y/N" in save_out or "yes/no" in save_out.lower() or "?" in save_out:
            connection.write_channel("y\n")
            time.sleep(2)
            _expect(connection, CLI_PROMPT_PATTERN, HOP_TIMEOUT, "prompt after save confirmation")
        ...
        if verified == "No":
            status = "Warning"
            message = f"Saved & upload triggered via hop through {intermediary_ip}, but file not found in TFTP root"
        else:
            status = "Success"
            message = f"Saved & backed up via hop through {intermediary_ip}" + (" (succeeded on retry)" if retried else "")
...
def main():
    ...
    # Command-line argument support for Portal and CLI automation
    parser = argparse.ArgumentParser(description="Extreme Switch Backup with Mandatory Save")
    parser.add_argument("--switch", "-s", type=str, help="Target specific switch IP to backup (single switch mode)", default=None)
    parser.add_argument("--all", "-a", action="store_true", help="Backup all switches from Switches.txt")
    parser.add_argument("target", nargs="?", default=None, help="Target switch IP (optional positional argument)")
    args, _ = parser.parse_known_args()

    target_ip = args.switch or args.target

    if target_ip and target_ip.upper() != "ALL":
        switches = [target_ip.strip()]
        logging.info(f"Targeting single switch from argument: {target_ip.strip()}")
    else:
        switches = read_switches(SWITCH_FILE)

    if not switches:
        logging.error("No switches found. Check Switches.txt or provide a target switch IP.")
        update_status("BackupSave.py", "FAILED", action="No switches found in Switches.txt", started_at=started_at_str)
        return
    ...
    update_status("BackupSave.py", "RUNNING", index=0, total=total, action="Starting backup process", started_at=started_at_str)

    logging.info("=" * 60)
    logging.info("Starting Extreme switch backup WITH MANDATORY SAVE")
    ...
    for index, ip in enumerate(switches, start=1):
        ...
        update_status(
            "BackupSave.py", "RUNNING", current_switch=ip, index=index, total=total,
        ...
        if False:
            logging.error(f"[{index}/{total}] {ip} - Ping failed. Skipping.")
            results.append(base_result(
                ip, ping="No", port_open="Not checked",
                backup_status="Skipped", message="Ping failed"
            ))
            continue

        logging.info(f"[{index}/{total}] {ip} - Ping OK. Checking TCP/{port}...")

        port_ok = tcp_port_open(ip, port)

        if not port_ok:
            logging.error(f"[{index}/{total}] {ip} - TCP/{port} closed or unreachable. Skipping.")
            results.append(base_result(
                ip, ping="Yes", port_open="No",
                backup_status="Skipped", message=f"TCP/{port} closed or unreachable"
            ))
            continue

        logging.info(f"[{index}/{total}] {ip} - TCP/{port} open. Saving config & backing up.")

        result = backup_switch(ip)
        results.append(result)

        if result["backup_status"] == "Success":
            logging.info(f"[{index}/{total}] {ip} - Save & Backup success.")
        elif result["backup_status"] == "Warning":
            logging.warning(f"[{index}/{total}] {ip} - Warning: {result['message']}")
        else:
            logging.error(f"[{index}/{total}] {ip} - Failed: {result['message']}")

    if HOP_ENABLED:
        update_status(
            "BackupSave.py", "RUNNING - HOP PASS", index=total, total=total,
    ...
    saved_count = sum(1 for r in results if r["config_saved"] == "Yes")
    ...
    update_status(
        "BackupSave.py", "COMPLETED", index=total, total=total,
    ...
    logging.info(f"Configurations Saved:{saved_count}")
