...
=============================================================================
‚úÖ PORT {clean_port} BOUNCE COMPLETED SUCCESSFULLY!"""
        ...
        while True:
            try:
                ...
                if should_run:
                    ...
                    print(f"‚è∞ [Python Scheduler Daemon] Executing scheduled backup at {now.strftime('%Y-%m-%d %H:%M:%S')} (target: {target_time})")
...
class PortalHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        """
        ========================================================================
        üìå DEVELOPER GUIDE: BACKEND GET API ROUTES (portal_server.py)
        ========================================================================
    ...
    def do_POST(self):
        """
        ========================================================================
        üìå DEVELOPER GUIDE: BACKEND POST API ROUTES (portal_server.py)
        ========================================================================
  ...
  <div id="modal-portal-login" class="fixed inset-0 z-50 bg-slate-950/90 backdrop-blur-md flex items-center justify-center p-4">
    <div class="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden p-6 space-y-5 animate-in fade-in zoom-in-95 duration-200">
      ...
      <form onsubmit="handlePortalLoginSubmit(event)" class="space-y-4">
        ...
        >
          <span>üöÄ Sign In &amp; Start Session</span>
        </button>
      </form>
    ...
    <header class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-5">
      <div>
        <h1 class="text-2xl font-black text-emerald-400 flex items-center gap-2.5 tracking-tight">
          ‚ö° Extreme Switch Backup Portal
        </h1>
      ...
      <div class="flex items-center gap-2 flex-wrap">
        ...
        <div id="portal-user-badge" class="hidden flex items-center gap-2 bg-slate-900 border border-slate-700 px-3 py-1.5 rounded-xl text-xs font-mono">
          <span class="text-indigo-400">üë§</span>
          ...
          <button onclick="handlePortalLogout()" class="text-slate-400 hover:text-rose-400 ml-1" title="Sign Out">üö™ Sign Out</button>
        </div>
        ...
        <button onclick="openSwitchReplacementModal()" class="bg-indigo-600 hover:bg-indigo-500 text-white px-3.5 py-2 rounded-xl text-xs font-bold shadow-lg shadow-indigo-600/30 transition flex items-center gap-1.5 cursor-pointer">
          <span>üîÑ Switch Replacement Hub</span>
        </button>
        <button onclick="openVisualNodeGraphModal('YORK')" class="bg-indigo-950/90 hover:bg-indigo-900 text-indigo-200 border border-indigo-600/60 px-3.5 py-2 rounded-xl text-xs font-bold shadow transition flex items-center gap-1.5 cursor-pointer">
          <span class="text-indigo-400">üó∫Ô∏è</span>
        ...
        <button id="btn-top-rollout" onclick="openRolloutAuth()" class="hidden bg-amber-600 hover:bg-amber-500 text-white px-3.5 py-2 rounded-xl text-xs font-bold shadow-lg shadow-amber-600/30 transition flex items-center gap-2" style="display: none !important;">
          <span>üõ°Ô∏è Rollout Configuration</span>
        </button>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-3.5 font-mono">
        <div class="bg-slate-950 p-4 rounded-xl border border-slate-800 hover:border-slate-700 transition flex items-start justify-between gap-3">
          <div class="space-y-1 min-w-0">
            ...
            <div id="estate-last-summary" class="text-xs text-emerald-400">‚úî 100% Complete ‚Ä¢ Save Config &amp; TFTP Export</div>
          </div>
        ...
        <div class="bg-slate-950 p-4 rounded-xl border border-slate-800 hover:border-slate-700 transition flex items-start justify-between gap-3">
          <div class="space-y-1 min-w-0">
            <div class="flex items-center justify-between">
              ...
              >
                <span>‚öôÔ∏è</span> Configure Schedule
              </button>
            </div>
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xl space-y-4">
          <div class="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-2 text-xs font-mono text-slate-400 shrink-0">
              <span id="active-site-tag" class="hidden px-2.5 py-1 rounded bg-indigo-500/20 text-indigo-300 border border-indigo-500/40 text-[11px] font-bold">
                ...
                <button onclick="selectSite(null)" class="ml-1 text-slate-400 hover:text-white">‚úï</button>
              </span>
          ...
          <div class="flex flex-wrap items-center justify-between gap-3 pt-2 border-t border-slate-800/80">
            ...
            <div class="flex items-center gap-2">
              ...
              >
                <span>üåê Test All Reachability</span>
              </button>
            </div>
          </div>
    ...
    <div class="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-5xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden">
      ...
      <div class="px-6 py-4 border-b border-slate-800 flex items-center justify-between bg-slate-950/90">
        <div class="flex items-center gap-3">
          ...
          <div>
            <div class="flex items-center gap-2 flex-wrap">
              ...
              >
                <span>üîÑ Profile: <span id="monitor-active-profile-label">EXOS Commands</span></span>
              </button>
            </div>
        ...
        <div class="flex items-center gap-2.5">
          ...
          <button onclick="closeMonitorModal()" class="text-slate-400 hover:text-white text-lg px-2">‚úï</button>
        </div>
      </div>
      ...
      <div class="px-6 py-3 border-b border-slate-800 bg-slate-950/60 flex flex-wrap items-center justify-between gap-3">
        <div class="flex items-center gap-1 bg-slate-950 p-1 rounded-lg border border-slate-800">
          ...
          <button id="monitor-tab-processes" onclick="switchMonitorTab('processes')" class="px-3.5 py-1 text-xs font-semibold rounded-md text-slate-400 hover:text-slate-200 transition">
            ‚öôÔ∏è Top Processes (<span id="modal-monitor-proc-count">6</span>)
          </button>
      ...
      <div class="p-6 overflow-y-auto flex-1 space-y-6">
        ...
        <div id="monitor-view-overview" class="space-y-6">
          ...
          <div class="bg-slate-950 border border-slate-800 rounded-xl p-4 flex flex-wrap items-center justify-between gap-4 font-mono text-xs text-slate-300">
            <div class="flex items-center gap-2">
              <span class="text-indigo-400">üõ°Ô∏è</span>
            ...
            <div class="flex items-center gap-2">
              <span class="text-indigo-400">‚è±Ô∏è</span>
        ...
        <div id="monitor-view-raw" class="hidden space-y-2">
          <div class="flex items-center justify-between text-xs font-mono">
            ...
            <button onclick="copyElementText('modal-monitor-raw-content')" class="text-indigo-400 hover:underline">üìã Copy CLI Output</button>
          </div>
        ...
        <div id="monitor-view-cmds" class="hidden space-y-4 font-mono text-xs">
          <div class="p-4 bg-slate-950 border border-slate-800 rounded-xl space-y-3">
            <h3 class="text-sm font-bold text-emerald-400 flex items-center gap-2">
              <span>‚ö° Extreme EXOS (Summit Series - X440, X460, X465, 5420, 5520) Commands</span>
            </h3>
          ...
          <div class="p-4 bg-slate-950 border border-slate-800 rounded-xl space-y-3">
            <h3 class="text-sm font-bold text-indigo-400 flex items-center gap-2">
              <span>üõ°Ô∏è Extreme VOSS (VSP Series - 4000, 7200, 7400, 8400) Commands</span>
            </h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-slate-300">
              <div class="bg-slate-900 p-3 rounded-lg border border-slate-800 space-y-1">
  ...
  <div id="modal-ports" class="hidden fixed inset-0 z-50 bg-slate-950/85 backdrop-blur-md flex items-center justify-center p-4">
    <div class="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
      <div class="px-6 py-4 border-b border-slate-800 flex items-center justify-between bg-slate-950/80">
        <div>
          <div class="flex items-center gap-2">
            <span class="text-emerald-400 font-bold">‚ö° Live Port Information Auditor</span>
        ...
        <button onclick="closeModal('modal-ports')" class="text-slate-400 hover:text-white text-lg px-2">‚úï</button>
      </div>
    ...
    <div class="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-5xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden">
      ...
      <div class="px-6 py-4 border-b border-slate-800 flex items-center justify-between bg-slate-950/90">
        ...
        <div class="flex items-center gap-3">
          ...
          <button onclick="closeModal('modal-lldp')" class="text-slate-400 hover:text-white text-lg px-2">‚úï</button>
        </div>
      </div>
      ...
      <div class="px-6 py-3 border-b border-slate-800 bg-slate-950/40 flex flex-wrap items-center justify-between gap-3">
        <div class="flex items-center gap-1 bg-slate-950 p-1 rounded-lg border border-slate-800">
          <button id="lldp-tab-table" onclick="switchLldpTab('table')" class="px-3 py-1 text-xs font-semibold rounded-md bg-indigo-600 text-white transition">
            üìä Structured Table (<span id="modal-lldp-count">0</span>)
          </button>
        ...
        <div class="relative flex-1 max-w-xs">
          ...
          <span class="absolute right-2.5 top-2 text-slate-500 text-xs">üîç</span>
        </div>
      </div>
      ...
      <div class="p-6 overflow-y-auto flex-1 space-y-4">
        ...
        <div id="lldp-view-raw" class="hidden space-y-2">
          <div class="flex items-center justify-between text-xs font-mono">
            ...
            <button onclick="copyElementText('modal-lldp-raw-content')" class="text-indigo-400 hover:underline">üìã Copy CLI Output</button>
          </div>
    ...
    <div class="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-5xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden">
      ...
      <div class="px-6 py-4 border-b border-slate-800 flex items-center justify-between bg-slate-950/90">
        ...
        <div class="flex items-center gap-3">
          ...
          <button onclick="closeModal('modal-fdb')" class="text-slate-400 hover:text-white text-lg px-2">‚úï</button>
        </div>
      </div>
      ...
      <div class="px-6 py-3 border-b border-slate-800 bg-slate-950/60 flex flex-wrap items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-1 bg-slate-950 p-1 rounded-lg border border-slate-800">
          <button id="fdb-tab-table" onclick="switchFdbTab('table')" class="px-3 py-1 text-xs font-semibold rounded-md bg-emerald-600 text-white transition">
            üìä MAC Table (<span id="modal-fdb-count">0</span>)
          </button>
        ...
        <div class="flex flex-wrap items-center gap-2 flex-1 justify-end">
          ...
          <div class="relative min-w-[200px] max-w-xs">
            ...
            <button onclick="clearFdbMacInput()" class="absolute right-2 top-1.5 text-slate-500 hover:text-slate-300 text-xs">‚úï</button>
          </div>
      ...
      <div class="p-6 overflow-y-auto flex-1 space-y-4">
        ...
        <div id="fdb-view-raw" class="hidden space-y-2">
          <div class="flex items-center justify-between text-xs font-mono">
            <span class="text-slate-400">Verbatim CLI Terminal Output &bull; Full Untruncated Buffer</span>
            <button onclick="copyElementText('modal-fdb-raw-content')" class="text-emerald-400 hover:underline">üìã Copy CLI Output</button>
          </div>
          <div class="bg-slate-950 p-4 rounded-xl border border-slate-800 font-mono text-xs text-slate-200 overflow-x-auto max-h-[500px]">
  ...
  <div id="modal-backups" class="hidden fixed inset-0 z-50 bg-slate-950/85 backdrop-blur-md flex items-center justify-center p-4">
    <div class="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-5xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden">
      <div class="px-6 py-4 border-b border-slate-800 flex items-center justify-between bg-slate-950/90">
        <div>
          <div class="flex items-center gap-2">
            <span class="text-indigo-400 font-bold text-sm">üïí Retained Backup Revisions & Configuration Archive</span>
        ...
        <div class="flex items-center gap-2">
          <button onclick="copyCurrentSelectedBackupRevision()" class="bg-indigo-600 hover:bg-indigo-500 text-white px-3.5 py-1.5 rounded-lg text-xs font-bold font-mono transition flex items-center gap-1.5 shadow">
            <span>üìã Copy Selected Config</span>
          </button>
          ...
          <button onclick="closeModal('modal-backups')" class="text-slate-400 hover:text-white text-lg px-2">‚úï</button>
        </div>
      </div>
      ...
      <div class="flex flex-col md:flex-row flex-1 min-h-0 overflow-hidden">
        ...
        <div class="w-full md:w-80 border-b md:border-b-0 md:border-r border-slate-800 bg-slate-950/70 p-3.5 flex flex-col shrink-0">
          <div class="text-[11px] font-bold uppercase tracking-wider text-slate-400 font-mono mb-2 flex items-center justify-between">
            <span>üìÇ Retained Files (<span id="modal-revisions-count">0</span>)</span>
    ...
    <div class="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-5xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden">
      ...
      <div class="px-6 py-4 border-b border-slate-800 flex items-center justify-between bg-slate-950/90">
        ...
        <div class="flex items-center gap-2">
          ...
          <button onclick="closeModal('modal-cheatsheet')" class="text-slate-400 hover:text-white text-lg px-2">‚úï</button>
        </div>
      </div>
  ...
  <div id="modal-bounce-port" class="hidden fixed inset-0 z-50 bg-slate-950/85 backdrop-blur-md flex items-center justify-center p-4">
    <div class="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-3xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden">
      ...
      <div class="px-6 py-4 border-b border-slate-800 flex items-center justify-between bg-slate-950/90">
        <div>
          <div class="flex items-center gap-2">
            <span class="p-1 rounded-md bg-amber-500/20 text-amber-400 border border-amber-500/30 text-xs font-mono font-bold">‚ö° PORT BOUNCE</span>
        ...
        <button onclick="closeModal('modal-bounce-port')" class="text-slate-400 hover:text-white text-lg px-2">‚úï</button>
      </div>
        ...
        <div class="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-3">
          <div class="flex items-center justify-between">
            <label class="text-xs font-bold uppercase tracking-wider text-slate-300 font-mono flex items-center gap-1.5">
              <span>üîå 1. Select Port Number to Bounce</span>
            </label>
        ...
        <div class="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-3">
          <div class="flex items-center justify-between">
            <h3 class="text-xs font-bold uppercase tracking-wider text-slate-300 font-mono flex items-center gap-1.5">
              <span>üè∑Ô∏è 2. Learned MAC Address(es) on Port <span id="bounce-active-port-label" class="text-amber-400 font-bold">13</span></span>
            </h3>
        ...
        <div id="bounce-result-card" class="hidden bg-slate-950 p-4 rounded-xl border border-emerald-800 space-y-2">
          <div class="flex items-center justify-between text-xs font-mono">
            <span class="text-emerald-400 font-bold flex items-center gap-1.5">
              <span>‚úî Port Bounce Completed Successfully</span>
            </span>
            <button type="button" onclick="copyElementText('bounce-result-cli')" class="text-slate-400 hover:text-emerald-400">üìã Copy Log</button>
          </div>
      ...
      <div class="px-6 py-3.5 border-t border-slate-800 bg-slate-950/80 flex items-center justify-between">
        ...
        <button type="button" id="btn-execute-bounce" onclick="executeBouncePortLive()" class="flex items-center gap-2 px-5 py-2.5 rounded-lg text-xs font-bold bg-amber-600 hover:bg-amber-500 text-white shadow-lg transition-all">
          <span>‚ö° Confirm &amp; Bounce Port <span id="btn-bounce-port-num">13</span></span>
        </button>
      </div>
  ...
  <div id="modal-rollout-auth" class="hidden fixed inset-0 z-50 bg-slate-950/85 backdrop-blur-md flex items-center justify-center p-4">
    <div class="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-md shadow-2xl p-6 space-y-5">
      <div class="flex items-center justify-between border-b border-slate-800 pb-4">
        ...
        <button onclick="closeModal('modal-rollout-auth')" class="text-slate-400 hover:text-white p-1">‚úï</button>
      </div>
      ...
      <div class="space-y-3">
        ...
        <form onsubmit="submitRolloutAuth(event)" class="space-y-4 pt-2">
          <div class="space-y-1.5">
            <label class="text-xs font-semibold text-slate-300 font-mono flex items-center gap-1.5">
              <span>üîë Enter Administrator Password:</span>
            </label>
            <input
              type="password"
          ...
          <div id="rollout-auth-error" class="hidden p-3 rounded-xl bg-rose-950/60 border border-rose-800 text-rose-300 text-xs font-mono">
            ‚ùå Incorrect password. Access denied. Returning to main page...
          </div>
  ...
  <div id="modal-rollout-workspace" class="hidden fixed inset-0 z-50 bg-slate-950/85 backdrop-blur-md flex items-center justify-center p-4">
    <div class="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-5xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden">
      ...
      <div class="px-6 py-4 border-b border-slate-800 flex items-center justify-between bg-slate-950/90">
        ...
        <button onclick="closeModal('modal-rollout-workspace')" class="text-slate-400 hover:text-white text-lg px-2">‚úï</button>
      </div>
      ...
      <div class="p-6 overflow-y-auto flex-1 space-y-6">
        ...
        <div id="rollout-composer-view" class="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div class="lg:col-span-7 space-y-4">
            ...
            <div class="space-y-2">
              <div class="flex items-center justify-between text-[11px] font-mono text-slate-400">
                <span class="font-bold text-slate-300">‚ú® Quick Command Templates:</span>
          ...
          <div class="lg:col-span-5 space-y-4 flex flex-col">
            <div class="flex items-center justify-between">
              <label class="text-xs font-bold uppercase tracking-wider text-slate-300 font-mono flex items-center gap-2">
                <span>üñ•Ô∏è 2. Target Devices (<span id="rollout-selected-count">0</span>)</span>
              </label>
            </div>
          ...
          <div class="p-4 rounded-xl bg-slate-950 border border-emerald-800 flex items-center justify-between">
            <div>
              <div class="text-sm font-bold text-white flex items-center gap-2">
                <span>‚úî Fleet Rollout Completed</span>
              </div>
      ...
      <div class="px-6 py-4 border-t border-slate-800 bg-slate-950/80 flex items-center justify-between">
        ...
        >
          <span>üöÄ Execute Rollout on Selected Switches</span>
        </button>
      </div>
  ...
  <div id="modal-switches-editor" class="hidden fixed inset-0 z-50 bg-slate-950/85 backdrop-blur-md flex items-center justify-center p-4">
    <div class="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-4xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden">
      ...
      <div class="px-6 py-4 border-b border-slate-800 flex items-center justify-between bg-slate-950/90">
        ...
        <button onclick="closeModal('modal-switches-editor')" class="text-slate-400 hover:text-white text-lg px-2">‚úï</button>
      </div>
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-2">
            <label class="px-3 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 text-xs font-mono cursor-pointer transition">
              <span>üìÅ Import / Upload File</span>
      ...
      <div class="px-6 py-4 border-t border-slate-800 bg-slate-950/80 flex items-center justify-between">
        ...
        >
          <span>üíæ Save &amp; Reload Fleet</span>
        </button>
      </div>
  ...
  <div id="modal-ping" class="hidden fixed inset-0 z-50 bg-slate-950/85 backdrop-blur-md flex items-center justify-center p-4">
    <div class="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
      ...
      <div class="px-6 py-4 border-b border-slate-800 flex items-center justify-between bg-slate-950/90">
        ...
        <button onclick="closeModal('modal-ping')" class="text-slate-400 hover:text-white text-lg px-2">‚úï</button>
      </div>
  ...
  <div id="modal-schedule" class="hidden fixed inset-0 z-50 bg-slate-950/85 backdrop-blur-md flex items-center justify-center p-4">
    <div class="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-3xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden font-mono">
      ...
      <div class="px-6 py-4 border-b border-slate-800 flex items-center justify-between bg-slate-950/90">
        ...
        <button onclick="closeModal('modal-schedule')" class="text-slate-400 hover:text-white text-lg px-2">‚úï</button>
      </div>
        ...
        <div class="space-y-2">
          ...
          <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
            <button type="button" onclick="setSchedFreq('daily')" id="freq-btn-daily" class="p-3 rounded-xl border text-left transition flex flex-col justify-between gap-1 bg-indigo-950/40 border-indigo-500/50 text-indigo-300">
              <span class="font-bold">üåô Daily Nightly</span>
            ...
            <button type="button" onclick="setSchedFreq('hourly')" id="freq-btn-hourly" class="p-3 rounded-xl border text-left transition flex flex-col justify-between gap-1 bg-slate-950 border-slate-800 text-slate-300 hover:border-slate-700">
              <span class="font-bold">‚è±Ô∏è Hourly</span>
            ...
            <button type="button" onclick="setSchedFreq('every_4h')" id="freq-btn-every_4h" class="p-3 rounded-xl border text-left transition flex flex-col justify-between gap-1 bg-slate-950 border-slate-800 text-slate-300 hover:border-slate-700">
              <span class="font-bold">‚ö° Every 4 Hours</span>
            ...
            <button type="button" onclick="setSchedFreq('weekly')" id="freq-btn-weekly" class="p-3 rounded-xl border text-left transition flex flex-col justify-between gap-1 bg-slate-950 border-slate-800 text-slate-300 hover:border-slate-700">
              <span class="font-bold">üìÖ Weekly</span>
      ...
      <div class="px-6 py-4 border-t border-slate-800 bg-slate-950/80 flex items-center justify-between font-mono text-xs">
        ...
        <div class="flex items-center gap-2">
          ...
          <button type="button" id="btn-save-schedule" onclick="saveScheduleModal()" class="px-5 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-xs font-bold text-white transition shadow-lg shadow-indigo-600/30 flex items-center gap-2">
            <span>üíæ Save Schedule &amp; Apply Policy</span>
          </button>
        </div>
      </div>
  ...
  <div id="modal-audit-trail" class="hidden fixed inset-0 z-50 bg-slate-950/85 backdrop-blur-md flex items-center justify-center p-4">
    <div class="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-5xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden">
      ...
      <div class="px-6 py-4 border-b border-slate-800 flex items-center justify-between bg-slate-950/90">
        ...
        <button onclick="closeModal('modal-audit-trail')" class="text-slate-400 hover:text-white text-lg px-2">‚úï</button>
      </div>
      ...
      <div class="px-6 py-3 border-b border-slate-800 bg-slate-900/60 flex flex-wrap items-center gap-3">
        ...
        <div class="flex items-center gap-2">
          ...
          >
            <span id="btn-toggle-all-audit-label">üìñ Expand All</span>
          </button>
          <button
      ...
      <div class="flex-1 relative overflow-hidden bg-slate-950 flex">
        ...
        <div id="modal-node-inspector" class="w-80 sm:w-96 border-l border-slate-800 bg-slate-900/95 flex flex-col justify-between hidden transition-all shadow-2xl z-20">
          <div class="p-4 border-b border-slate-800 flex items-center justify-between bg-slate-950/80">
            <div class="flex items-center gap-2">
              <span id="inspector-node-icon" class="text-base">üóÑÔ∏è</span>
              <div>
            ...
            <button onclick="closeNodeInspector()" class="text-slate-400 hover:text-white text-xs p-1">‚úï</button>
          </div>
          ...
          <div id="rma-workspace-content" class="space-y-5 hidden">
            ...
            <div class="bg-slate-950 border border-slate-800 rounded-xl overflow-hidden flex flex-col shadow-inner">
              <div class="px-4 py-2.5 bg-slate-900/90 border-b border-slate-800 flex items-center justify-between text-xs font-mono">
                ...
                <span class="text-emerald-400 text-[11px]">‚úî Ready for TFTP / Console Injection</span>
              </div>
...
WantedBy=multi-user.target</pre>
          <button onclick="copyToClipboard(`[Unit]\nDescription=Extreme Switch Backup & Management Portal Server\nAfter=network.target\n\n[Service]\nType=simple\nUser=root\nWorkingDirectory=/opt/switch-backup\nExecStart=/usr/bin/python3 /opt/switch-backup/portal_server_ubuntu.py\nRestart=always\nRestartSec=5\n\n[Install]\nWantedBy=multi-user.target`, 'Copied systemd unit file!')" class="px-3 py-1 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded text-xs">üìã Copy Unit File</button>
        </div>
      </div>
  ...
  <div id="toast" class="fixed bottom-6 right-6 bg-emerald-600 text-white px-4 py-2.5 rounded-xl shadow-2xl text-xs font-bold font-mono transition-opacity duration-300 opacity-0 pointer-events-none z-50 flex items-center gap-2">
    <span>‚úî</span> <span id="toast-msg">Copied to clipboard!</span>
  </div>
  ...
  <script>
          ...
          <div class="mx-4 mb-4 grid grid-cols-1 lg:grid-cols-3 gap-4 pb-4">
            <div class="lg:col-span-2 bg-slate-950 rounded-xl border border-slate-800 p-4 space-y-3">
              <div class="flex items-center justify-between border-b border-slate-800/80 pb-2.5">
                <div class="flex items-center gap-2">
                  <span class="text-emerald-400 font-bold">üì°</span>
            ...
            <div class="flex items-start justify-between gap-2">
              <div>
                ...
                <div class="flex items-center gap-2 mt-1.5">
                  ...
                  >
                    <span>${sw.ip}</span>
                    <span class="text-[10px] text-slate-500">üìã</span>
                  </button>
                </div>
              </div>
              ...
              <div class="flex flex-col items-end gap-1 shrink-0">
                ...
                <span class="text-[10px] font-mono ${sw.hasBackup ? 'text-emerald-400' : 'text-slate-500'}">
                  ${sw.hasBackup ? '‚úî Backed Up' : 'No Backup'}
                </span>
        ...
        <div id="site-page-banner" class="space-y-6">
          ...
          <div class="bg-slate-900/95 border border-slate-800 rounded-2xl p-5 shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-2.5 flex-wrap">
              ...
              >
                <span>üîÑ Switch Replacement</span>
              </button>
              <button
              ...
              >
                <span>‚ö° Backup ${selectedSite} Switches</span>
              </button>
              <button
                onclick="selectSite(null)"
          ...
          <div class="bg-slate-900 border border-slate-800 rounded-xl p-1.5 flex items-center justify-between gap-2 overflow-x-auto shadow-md">
            <div class="flex items-center gap-2 min-w-max">
              ...
              >
                <span>üó∫Ô∏è Visual Node Graph</span>
              ...
              >
                <span>üì∂ Wireless RF Heatmaps</span>
              </button>
              ...
              >
                <span>üóÑÔ∏è Switch Fleet (${siteSwitches.length})</span>
              </button>
          ...
          ${currentSiteTab === 'graph' ? `
            <div class="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-xl space-y-4 p-4">
              <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-800">
                <div class="flex items-center gap-2.5">
                  <span class="text-indigo-400 font-mono text-lg">üó∫Ô∏è</span>
                  <div>
                ...
                <div class="flex items-center gap-2 flex-wrap">
                  <button onclick="exportTopologyJson('${selectedSite}')" class="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-mono font-semibold rounded-lg border border-slate-700 transition flex items-center gap-1">
                    <span>üíæ Export Topology JSON</span>
                  </button>
                </div>
              </div>
          ...
          ${currentSiteTab === 'blueprint' ? `
            <div class="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-xl">
              <div class="p-3.5 bg-slate-900/90 border-b border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div class="flex items-center gap-2.5">
                  <span class="text-indigo-400 font-mono text-sm">üìê</span>
                  <div>
                    <div class="flex items-center gap-2">
                    ...
                    return `
                      <div class="w-full flex flex-col items-center space-y-3">
                        <div class="w-full bg-slate-900/90 p-2.5 rounded-xl border border-slate-800 flex items-center justify-between text-xs font-mono text-slate-300">
                          <div class="flex items-center gap-2">
                            <span class="text-emerald-400 font-bold">‚úî</span>
              ...
              ${isYork ? `
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 p-3 bg-slate-950 border-t border-slate-800 text-xs font-mono">
                  <div class="bg-slate-900 p-2.5 rounded-lg border border-slate-800">
                    ...
                    <div class="text-slate-400 text-[11px] mt-0.5">Core Port 9 ‚ûî Port 1</div>
                  </div>
                  <div class="bg-slate-900 p-2.5 rounded-lg border border-slate-800">
                    <div class="text-purple-400 font-bold">DLC-York-Gym</div>
                    <div class="text-slate-400 text-[11px] mt-0.5">Core Port 37 ‚ûî Port 1</div>
                  </div>
                  <div class="bg-slate-900 p-2.5 rounded-lg border border-slate-800">
                    <div class="text-purple-400 font-bold">DLL-York</div>
                    <div class="text-slate-400 text-[11px] mt-0.5">Core Port 42 ‚ûî Port 17</div>
                  </div>
                  <div class="bg-slate-900 p-2.5 rounded-lg border border-slate-800">
                    <div class="text-purple-400 font-bold">DLC-York-MainComms-2</div>
                    <div class="text-slate-400 text-[11px] mt-0.5">Core Port 41 ‚ûî Port 48</div>
                  </div>
                </div>
              ` : ''}
            </div>
                ...
                ${siteSwitches.map(sw => `
                  <div class="bg-slate-950 border border-slate-800 hover:border-indigo-500/60 rounded-xl p-4 flex flex-col justify-between space-y-4 shadow transition">
                    ...
                    <div class="flex items-center gap-2 pt-2 border-t border-slate-900">
                      ...
                      >
                        <span>üîÑ Launch Replacement</span>
                      </button>
                      <button
                        onclick="runBackup('${sw.ip}')"
        ...
        return `
          ...
          >
            ...
            <div class="flex items-center justify-between text-[11px] font-mono text-slate-400 mt-1">
              <span>${sw.ip}</span>
              <span class="${sw.hasBackup ? 'text-emerald-400' : 'text-slate-500'}">${sw.hasBackup ? '‚úî Backup' : 'No Backup'}</span>
            </div>
          </div>
        `;
      }).join('');
    ...
    function loadSiteDiagramInModal(siteCode) {
      ...
      if (pngPath) {
        viewport.innerHTML = `
          <div class="w-full flex flex-col items-center space-y-4">
            <div class="flex items-center gap-3">
              <a href="${pngPath}" target="_blank" class="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-xs font-mono font-bold transition">Open Full Resolution ‚Üó</a>
              <a href="${pngPath}" download="${siteCode}_Topology.png" class="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-lg text-xs font-mono font-bold transition">Download PNG üíæ</a>
            </div>
          ...
          <div class="rounded-xl transition-all border ${isSelected ? 'bg-indigo-950/40 border-indigo-500/50 shadow-sm' : 'bg-slate-950/40 hover:bg-slate-950/80 border-slate-800/60'}">
            ...
            >
              <div class="flex items-center gap-2 min-w-0 flex-1">
                ...
                >
                  <span>${site}</span>
                  <span class="text-[10px] text-indigo-400/80">‚Üó</span>
                </a>
              </div>
    ...
    async function fetchStatus() {
      try {
        ...
        if (data.schedule) {
          ...
          if (freqEl) {
            freqEl.innerText = `${sch.scheduleFrequency || 'Daily Nightly Backup (02:00)'} ‚Ä¢ ${sch.scheduleEngine || 'switch-backup.timer'}`;
          }
          if (lastSum) {
            lastSum.innerText = `‚úî ${allSwitches.length || 6}/${allSwitches.length || 6} Switches Saved ‚Ä¢ TFTP + SSH`;
          }
          if (lastBdg) {
    ...
    async function copySwitchBackup(ip, hostname) {
      try {
        ...
        if (data && data.backupContent) {
          ...
          showToast(`No backup file found on disk for ${ip}. Click "‚ö° Backup" first!`);
        }
      } catch (err) {
    ...
    function renderMonitorModalData(data) {
      ...
      if (cpuLoadsEl) {
        cpuLoadsEl.innerText = `5s: ${cpu}% ‚Ä¢ 1m: ${(cpu * 0.95).toFixed(1)}% ‚Ä¢ 5m: ${(cpu * 0.92).toFixed(1)}%`;
      }
        ...
        procTbody.innerHTML = processes.map(p => `
          <tr class="hover:bg-slate-900/60 transition">
            ...
            <td class="py-2.5 px-4 text-white font-bold font-mono flex items-center gap-1.5">
              <span>‚öôÔ∏è</span> <span>${p.name}</span>
            </td>
    ...
    function copyCurrentSelectedBackupRevision() {
      ...
      navigator.clipboard.writeText(text).then(() => {
        ...
        showToast(`‚úî Copied ${fname} to clipboard!`, 'success');
      }).catch(err => {
    ...
    function downloadCurrentSelectedBackupRevision() {
      ...
      showToast(`üì• Downloading ${fname}`, 'success');
    }
    ...
    function copyCheatSheetStep(stepId) {
      ...
      navigator.clipboard.writeText(preEl.innerText).then(() => {
        showToast('‚úî Commands copied to clipboard!', 'success');
      }).catch(err => {
        showToast('Failed to copy: ' + err, 'error');
      });
    }
          ...
          <tr class="hover:bg-slate-900/60 transition cursor-pointer group" onclick="toggleNeighborDetails(${idx})">
            ...
            <td class="py-3 px-3">
              <div class="font-bold text-slate-100 flex items-center gap-1.5">
                <span>${isCoreOrSwitch ? 'üîÄ' : (isAp ? 'üì°' : 'üíª')}</span>
            ...
            <td class="py-3 px-3">
              ...
              ${n.poe ? `<div class="text-[10px] text-amber-400 font-mono">‚ö° ${n.poe}</div>` : ''}
            </td>
            ...
            <td colspan="7" class="p-4">
              <div class="bg-slate-900 border border-slate-800 rounded-xl p-4 space-y-3 font-mono text-xs">
                ...
                <div class="pt-2 border-t border-slate-800/80 flex justify-between items-center text-[11px]">
                  ...
                  <button onclick="copyLldpNeighborByIndex(${idx})" class="text-indigo-400 hover:underline">üìã Copy Neighbor Block</button>
                </div>
              </div>
            </td>
          </tr>
      ...
      if (!uplinkNeighbors.length) {
        ...
        uplinksContainer.innerHTML = uplinkNeighbors.map(u => `
          <div class="bg-slate-950 border border-slate-800 rounded-xl p-4 space-y-2 font-mono text-xs">
            <div class="flex items-center justify-between">
              <span class="font-bold text-white flex items-center gap-1.5">üîÄ ${u.systemName}</span>
    ...
    async function showLldpNeighborsLive(ip, hostname) {
      ...
      document.getElementById('modal-lldp-subtitle').innerText = `Command: show lldp neighbors detailed ‚Ä¢ Switch ${hostname} (${ip})`;
      ...
      try {
        ...
        document.getElementById('modal-lldp-channel-status').innerText = `Live Telnet Channel: ${ip}:23 ‚Ä¢ Response: Active ‚Ä¢ Full Detailed Output`;
          ...
          <tr class="hover:bg-slate-900/60 transition group font-mono text-xs">
            <td class="py-2.5 px-3">
              <div class="flex items-center gap-1.5">
                ...
                <button onclick="copyToClipboard('${e.mac}', 'Copied MAC ${e.mac}');" class="text-slate-500 hover:text-emerald-400 text-[11px]" title="Copy MAC">üìã</button>
              </div>
            </td>
            <td class="py-2.5 px-3">
    ...
    async function fetchFdbData(ip, hostname, port = "ALL", mac = "") {
      ...
      try {
        ...
        document.getElementById('modal-fdb-channel-status').innerText = `Live Telnet Channel: ${ip}:23 ‚Ä¢ Response: Active ‚Ä¢ Command: ${data.command || 'show fdb'}`;
    ...
    async function showFdbTableLive(ip, hostname) {
      ...
      document.getElementById('modal-fdb-subtitle').innerText = `CLI: show fdb ‚Ä¢ ExtremeXOS MAC Table & Port Lookup`;
    ...
    function showSwitchPingLive(ip, hostname) {
      ...
      document.getElementById('modal-ping-subtitle').innerText = `Target IP: ${ip} ‚Ä¢ ICMP Network Latency & Reachability Test`;
    ...
    async function executePingModal() {
      ...
      btn.disabled = true;
      btn.innerHTML = `<span class="animate-spin mr-1">‚öôÔ∏è</span> Pinging...`;
      ...
      try {
        ...
        btn.disabled = false;
        btn.innerHTML = `<span>‚ö° Send Ping</span>`;
      }
    }
    ...
    function toggleCustomBouncePort() {
      ...
      if (isCustomBouncePort) {
        ...
        toggleBtn.innerText = '‚Üê Select from Standard List';
        ...
        if (matched.length === 0) {
          ...
          content.innerHTML = `
            <div class="p-4 rounded-xl bg-slate-900 border border-slate-800 flex items-start gap-3">
              <span class="text-slate-400 text-lg">‚ÑπÔ∏è</span>
          ...
          content.innerHTML = `
            <div class="p-4 rounded-xl bg-emerald-950/40 border border-emerald-700/60 flex items-start gap-3.5">
              <span class="text-emerald-400 text-xl font-bold">‚úî</span>
    ...
    async function executeBouncePortLive() {
      ...
      try {
        ...
        btn.innerHTML = `<span>‚úî Port ${currentBouncePort} Bounced</span>`;
        ...
        btn.disabled = false;
        btn.innerHTML = `<span>‚ö° Retry Bounce Port ${currentBouncePort}</span>`;
      }
    }
    ...
    function showBouncePortModal(ip, hostname, defaultPort = "13") {
      ...
      document.getElementById('modal-bounce-switch-sub').innerText = `Switch IP: ${ip} ‚Ä¢ Protocol: Telnet (Port 23) ‚Ä¢ CLI: disable port / enable port`;
      ...
      btn.innerHTML = `<span>‚ö° Confirm &amp; Bounce Port <span id="btn-bounce-port-num">${defaultPort}</span></span>`;
    ...
    function submitRolloutAuth(e) {
      ...
      if (enteredPassword === 'password') {
        ...
        setTimeout(() => {
          closeModal('modal-rollout-auth');
          showToast('‚ùå Incorrect password. Access denied.');
        }, 1200);
      }
    }
    ...
    function openRolloutWorkspace() {
      ...
      btn.disabled = false;
      btn.innerHTML = `<span>üöÄ Execute Rollout on Selected Switches</span>`;
...
save configuration`
      ...
      if (snippet) {
        textarea.value = (textarxúƒ=ÌnIrˇ˝mÆaígE˘k}ÙJéVíΩÃ…íNîˆêxjƒiä≥ŒÃŒá$FK‡ øí‡Ç‹ÂÄ,∏\^!@p?Ç<å_ ˜©ÍÓôÈÓÈ·á,€ﬁµ%ŒtWWWU◊WW7©›æ∞Ωî∂ì»ù4ö‰IËUbGT{˛Ä‘øÛøÛÎ§KÍu¸˚n“‰˘"˛§°c'Ù(º M∂É…ƒˆùÌ ıìF≥hèÉÀ„¿éìF˝òNBz€q®CíÄy'B7	¢zﬁmváˇÀ~ÿÒ‘íQÍ7	Ω¢√4vœΩ†ÄÏµË8¸8…'D6à”	ıìˆ9Mv=äø~=Ì9çzƒ˚[Öÿr˝0M
